...
function handleAudioTracksAdded(tech, shakaPlayer, tracks) {
  ...
  function findDashAudioTrack(subDashAudioTracks, videojsAudioTrack) {
    return subDashAudioTracks.find(
      (track) => generateLabelFromTrack(track) === videojsAudioTrack.label,
    )
  }
    ...
    for (let i = 0; i < videojsAudioTracks.length; i++) {
      ...
      if (track.enabled) {
        ...
        if (dashAudioTrack) {
          ...
          shakaPlayer.selectAudioLanguage(
            dashAudioTrack.language,
            dashAudioTrack.role,
          )
...
export function setupAudioTracks(tech, shakaPlayer) {
  handleAudioTracksAdded(
    tech,
    shakaPlayer,
