function _getQuality(tech, shakaPlayer) {
  ...
  const sortedTracks = tracks
    ...
    .reduce(
      (accumulator, track) => {
        if (
          track.height !== accumulator.previousHeight ||
          (track.height === accumulator.previousHeight &&
            track.language !== accumulator.previousLanguage)
        ) {
      ...
      { previousHeight: null, previousLanguage: null, list: [] },
    ).list

  return sortedTracks
}

export function setupQualityTracks(tech, shakaPlayer) {
